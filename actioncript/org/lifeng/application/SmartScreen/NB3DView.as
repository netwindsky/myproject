package org.lifeng.application.SmartScreen
{
	...
	public class NB3DView extends Sprite
	{
		...
		private var _yh:int=576;
		public function NB3DView(bgpath:String=null)
		{
			super();
		...
		public function init(str:String):void{
			...
			var file1:File=new File(File.applicationDirectory.nativePath);
			var file2:File=file1.parent;
			
			trace(file2.nativePath+"/3d/"+path+"/");
			///return;
			var arr:Array=file.getFilesScanFolder(new File(file2.nativePath+"/3d/"+path+"/"),["jpg","png"]);
			count=arr.length;
			
			for(var i:int=0;i<count;i++){
				var f:File=new File(file2.nativePath+"/3d/"+path+"/");
...
/*
		...
		public function operate(x:int,y:int,scale:Number):void{
			trace(x,y,scale,image.width,image.height);
			//image.scaleX=image.scaleY=1;
			trace(x,y,scale,image.width,image.height,_xw);
			offsetx=x//*xxxs//+(this.stage.stageWidth-_xw)*.5;
			offsety=y//*xxxs;
			image.x=offsetx;
			image.y=offsety;
			
			
			image.scaleX=image.scaleY=scale//*oscale;
