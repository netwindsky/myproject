...
package org.lifeng.tool
{
	...
	public class FullScreenScrollBar extends Sprite
	{
		...
		//============================================================================================================================
		public function FullScreenScrollBar(c:DisplayObjectContainer, tc:uint, gc:uint, gpc:uint, grip:uint, tt:int, gt:int, ea:int,height, hs:Boolean)
		//============================================================================================================================
		{
			_content = c;
			_trackColor = tc;
		...
		//============================================================================================================================
		{
			...
			trace(_cH - _tH,ty,_grabber.y,_scrollValue);
			dist = ty - _content.y + _defaultPosition;
			moveAmount = dist / _easeAmount;
			
			_content.y += Math.round(moveAmount);
			//trace(Math.abs(_content.y - ty - _defaultPosition));
			if (Math.abs(_content.y - ty - _defaultPosition) <= 2)
			{
				//trace("scrollContent");
		...
		//============================================================================================================================
		{
			...
			if(_content.height <_stageH) {
				...
				_content.removeEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelListener); 
			} else { 
				//stage.addEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelListener); 
				_content.addEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelListener); 
			}
		}
		...
		//============================================================================================================================
		{
			...
			//滚动条底
			var t:UIScrollbarbackground=new UIScrollbarbackground();
			//t.filters=[new GlowFilter(0x888888,1)]
			_track.addChild(t);
			addChild(_track);
		}
		...
		//============================================================================================================================
		{
			...
			if (d > 0)
			{
				...
				trace("mouseWheelListener");
				if (!_grabber.hasEventListener(Event.ENTER_FRAME)) _grabber.addEventListener(Event.ENTER_FRAME, scrollContent);
			}
			else
			{
				//if (((_grabber.y + _grabber.height) + (Math.abs(d) * 4)) <= stage.stageHeight)
				if (((_grabber.y + _grabber.height) + (Math.abs(d) * 4)) <= _stageH)
				{
		...
		{
			e.updateAfterEvent();
			trace("onMouseMoveListener");
			trace(_grabber.hasEventListener(Event.ENTER_FRAME))
			if (!_grabber.hasEventListener(Event.ENTER_FRAME)) _grabber.addEventListener(Event.ENTER_FRAME, scrollContent, false, 0, true);
		}
		...
		//============================================================================================================================
		{
			...
			else
			{
				...
				if(_grabber.y + _grabber.getChildByName("bg").height > _track.height) _grabber.y = _stageH - _grabber.getChildByName("bg").height;
			}
		...
		//============================================================================================================================
		{
			...
			_stageW = _content.width//stage.stageWidth;
			_stageH = _barHeight//stage.stageHeight;
			adjustSize();
		}
